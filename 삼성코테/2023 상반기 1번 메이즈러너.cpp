...
void map_rotate(int out_M) {
    ...
    for (int s = 0; s < cur_M; s++) {
        ...
        if (dist == dist_shortest) {
            if (x < x_short) { // 행>열 순으로 짧은 지점을 찾음(사람좌표)
                x_short = x;
                y_short = y;
            }
            else if (x == x_short) { // 행 길이 같으면 열이 짧은 사람
                if (y < y_short) {
                    x_short = x;
                    y_short = y;
                }
    ...
    while (K--) {
        ...
        for (int j = 0; j < cur_M; j++) { // 현재 사람들 각각에 대해서 수행
            ...
            for (int i = 0; i < 4; i++) { // 상하좌우 4방향 이동 수행
                ...
                if(new_dist < dist_shortest) dist_shortest = new_dist; // 짧은 거리 저장

                //cout << "cur_x,y " << cur_x << " " << cur_y << " " << "dist_shortest " << dist_shortest << "\n";

                person_coord.push({ new_x, new_y }); // 바뀐 좌표 저장
                map[new_x][new_y] = -2; // map변경
                map[cur_x][cur_y] = 0;
                break;
            }
            if (cur_dist < dist_shortest) dist_shortest = cur_dist; //person - exit 짧은 거리 저장
